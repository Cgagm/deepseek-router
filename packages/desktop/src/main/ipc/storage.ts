...
export function setupStorageIPC(): void {
  ...
  ipcMain.handle('storage:getChats', () => {
    ...
    const result = db.exec(
      'SELECT id, title, messages, created_at, updated_at, pinned FROM chat_sessions ORDER BY updated_at DESC',
    )
  ...
  ipcMain.handle('storage:getChat', (_event, id: string) => {
    const db = getDatabase()
    const result = db.exec(
      'SELECT id, title, messages, created_at, updated_at, pinned FROM chat_sessions WHERE id = ?',
      [id],
    )
  ...
  ipcMain.handle('storage:getSkills', () => {
    const db = getDatabase()
    const result = db.exec(
      'SELECT id, name, description, category, icon, prompt, tools FROM skills ORDER BY id',
    )
    if (!result.length) return []
  ...
  ipcMain.handle('storage:getTemplates', () => {
    const db = getDatabase()
    const result = db.exec(
      'SELECT id, name, description, category, content, icon FROM templates ORDER BY id',
    )
    if (!result.length) return []
    return result[0].values.map((row) => ({
      id: row[0],
